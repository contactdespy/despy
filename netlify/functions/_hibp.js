// ════════════════════════════════════════════
// DESPY — Vérification HIBP + Dark Web (définition unique, partagée)
// Have I Been Pwned API v3
//
// Ce module ne répond à aucune URL : il contient la mécanique, utilisée par
//   • hibp-check.js  — la surveillance de fond (cron du lundi 7h)
//   • hibp-manuel.js — le bouton « Vérifier mon email » de l'appli
//
// Pourquoi deux fonctions et pas une seule à deux modes : Netlify REFUSE au
// bord tout appel HTTP vers une fonction déclarée avec un `schedule`. La
// réponse est un 403 vide qui n'atteint jamais notre code — aucun log, aucune
// trace. Le bouton de l'appli a donc été mort du 8 juin au 26 août sans que
// rien ne le signale. Une fonction planifiée ne peut PAS servir aussi de point
// d'entrée client : il en faut une seconde, non planifiée.
// ════════════════════════════════════════════
...
// Appelée par hibp-manuel.js (bouton « Vérifier mon email » de l'appli).
//
// Renvoie un objet simple — pas une réponse HTTP : c'est l'endpoint qui décide
// du code de statut. Le champ `breaches` (nom + année de chaque fuite) existe
// pour que l'appli puisse AFFICHER lesquelles ; l'ancienne version ne renvoyait
// qu'un nombre, et l'appli, elle, cherchait une liste.
async function verifierEtHistoriser(supabase, rawEmail) {
  const email = (rawEmail || '').toLowerCase().trim();
  if (!email || !email.includes('@')) return { error: 'invalid_email', breachCount: null };
  ...
  try {
    ...
    if (breaches === null) return { error: 'unavailable', breachCount: null };
    ...
    // Les fuites les plus récentes d'abord : c'est celle de l'an dernier qui
    // sert à un escroc aujourd'hui, pas celle de 2013.
    const liste = breaches
      .map(b => ({ name: b.Name, date: b.BreachDate || '' }))
      .sort((a, b) => String(b.date).localeCompare(String(a.date)))
      .slice(0, 12);

    return { breachCount, breaches: liste, emailSent };
  } catch (e) {
    console.error('HIBP manuel error:', e.message);
    return { error: 'server', breachCount: null };
  }
}

module.exports = {
  checkEmailBreaches,
  buildBreachAlertHTML,
  alertTrustedContact,
  sendBreachPush,
  verifierEtHistoriser,
  // Module partagé, pas un endpoint : accès direct → 404 propre.
  handler: async () => ({ statusCode: 404, body: 'Not found' })
};
